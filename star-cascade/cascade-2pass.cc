...
/**
 *  This is an experimental version of the cascade code that processes
 *  the feature pyramid is two passes.  The first pass only computes detections
 *  on every other scale.  The 2nd pass computes detections on scales adjacent
 *  to scales that reported detections in the first pass (if any).  In practice
 *  this causes a minor decrease in precision and recall at the tail of the PR
 *  curve and increases detection speed by a factor of about 1.25 - 1.75x.
 */
...
static inline double conv(int x, int y, 
                          const double *A, const mwSize *A_dims,
                          const double *B, const mwSize *B_dims,
                          int num_features) {
  double val = 0;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  int *dolevel = new int[nlevels];

  // process each model component and pyramid level (note: parallelize here)
  for (int comp = 0; comp < MODEL->numcomponents; comp++) {
    // mark every other level for processing in the first pass
    for (int i = 0; i < nlevels; i++)
      dolevel[i] = (i % 2) ? 1 : 0;
    // go over feature pyramid in two passes:
    //  - the first pass find detections on every other level
    //  - the second pass only looks at level adjacent to levels 
    //    with detections above threshold
    for (int pass = 1; pass <= 2; pass++) {
      for (int plevel = 0; plevel < nlevels; plevel++) {
        if (dolevel[plevel] != pass)
          continue;
